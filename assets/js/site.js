...
const GOOGLE_ADS_ID = 'AW-18109380838';
const GOOGLE_ADS_WHATSAPP_SEND_TO = 'AW-18109380838/Jk3_CKzmx70cEObxnLtD';

// Reuse the Google tag already loaded by the website and add Google Ads as a destination.
...
function trackContact(eventName, extra = {}, callback) {
  ...
  if (typeof gtag === 'function') {
    try {
      // Keep the original GA4 event tracking.
      gtag('event', eventName, Object.assign({}, payload, {
        event_callback: done,
        event_timeout: 1200
      }));

      // Send a Google Ads conversion only for WhatsApp contact actions.
      if (extra && extra.contact_method === 'whatsapp') {
